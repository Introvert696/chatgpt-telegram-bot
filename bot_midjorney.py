...
def getMidjorneyToken():
    f = open(MIDJORNEY_TOKEN_PATH, "r")
    token = f.readlines()
    f.close()
    return token[0]
...
welcome_message = "Привет, это Бот ChatGPT. Здесь вы можете задать любой вопрос и получить на него ответ. Так же вы можете сгенерировать картинку через ChatGpt командой /image (текст запроса) ,а так же картинку через Midjorney /picture (текст запроса). "
...
# функция для генерации фото midjorney
def generate_image_midjorney(prompt):
    try:
        out_image = replicate.run(
...
# функция для генерации фото Chatgpt


def generate_image_chatgpt(prompt):
    try:
...
def command_message(message):
    ...
    if (protect):
        log_utils.inlog("Генерация фото Chatgpt", message.from_user.username,
                        message.from_user.id, message.text)
        prompt = message.text
        image = generate_image_chatgpt(prompt)
        bot.reply_to(message, text=image)

        log_utils.inlog("Генерация фото Chatgpt", "SERVER",
...
def command_message(message):
    ...
    if (protect):
        log_utils.inlog("Генерация фото Midjorney", message.from_user.username,
                        message.from_user.id, message.text)
        prompt = message.text
        image = generate_image_midjorney(prompt)
        bot.reply_to(message, text=image)

        log_utils.inlog("Генерация фото Midjorney", "SERVER",
                        0, "Фото успешно сгенерено пользователю - "+str(message.from_user.id))
